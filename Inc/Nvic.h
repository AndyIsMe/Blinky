...
struct NvicReg{
	...
	volatile uint32_t	STIR;							//Software Trigger Interrupt Register (e00h)
};

#define nivcEnableIrq(irqNum)	(Nvic->ISER[irqNum>>5] |= 1 << (irqNum & 0x1f))
#define nivcDisableIrq(irqNum)	(Nvic->ICER[irqNum>>5] |= 1 << (irqNum & 0x1f))
